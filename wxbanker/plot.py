...
try:
    ...
    from art.embeddedimage import PyEmbeddedImage
...
def _draw8Objects():
    # A graph with a bunch of points that are approximately square, and
    # a best fit curve to match.
    import random
    points = []
    for i in range(100):
        x = i
        #make the square off by plus or minus 50%
        variation = random.random() + .5
        y = (x**2) * variation
        points.append((x, y))
        
    line1 = PolyLine(points, colour='green')
    line2 = PolyBestFitLine(points, N=2, colour='blue', width=3)
    return PlotGraphics([line1, line2], "Data with a best fit curve", "Value X", "Value Y")

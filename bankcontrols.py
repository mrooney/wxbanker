...
class AccountListCtrl(wx.Panel):
    ...
    def SelectItem(self, index):
        ...
        pubsub.Publisher().sendMessage("ACCOUNT CHANGED", account)
    ...
    def RemoveAccount(self, accountName):
        """
        Actually remove an account from the database
        (and of course the list as well).
        """
        ...
        #tell everyone that cares
        pubsub.Publisher().sendMessage("REMOVED ACCOUNT", accountName)
    ...
    def onAddAccount(self, event):
        ...
        try:
            ...
            self.onHideEditCtrl()
            self._PutAccount(accountName)
            pubsub.Publisher().sendMessage("NEW ACCOUNT", accountName)
